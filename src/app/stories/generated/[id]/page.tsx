...
import { ArrowLeft, ArrowRight, BookOpen, Heart, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function GeneratedStoryPage() {
    ...
    const loadStory = async () => {
      try {
        ...
        if (response.ok) {
          ...
          readingTime: 8,
          ageGroup: [3, 4, 5, 6, 7, 8],          category: "adventure",
          createdAt: new Date()
        }
  ...
  const toggleFavorite = () => {
    ...
    toast({
      title: isFavorite 
        ? (state.language === "en" ? "Removed from favorites" : "Dihapus dari favorit")
        : (state.language === "en" ? "Added to favorites" : "Ditambahkan ke favorit"),
    })
  }
      ...
      <div className="min-h-screen bg-gradient-radial from-indigo-400 via-purple-500 to-pink-500 animate-gradient-slow flex items-center justify-center">
        <div className="text-center">
          <p className="text-white text-lg font-nunito">
            {state.language === "en" ? "Story not found" : "Cerita tidak ditemukan"}
          </p>
          <Button 
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <Heart className={`h-4 w-4 ${isFavorite ? "fill-current text-red-300" : ""}`} />
              </Button>
            </div>
          </div>
        </div>
