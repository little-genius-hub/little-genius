...
import { Calculator, BookOpen, Heart, User, LogOut, Star, Trophy, Globe, BarChart3 } from "lucide-react"
      ...
      <div className="w-full max-w-md mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            <Avatar className="w-12 h-12 border-2 border-white">
              {state.currentChild?.avatar ? (
                <AvatarImage src={state.currentChild.avatar} alt={state.currentChild.name} />
              ) : (
        ...
        <div className="grid grid-cols-2 gap-3">
          {gameMenuItems.map((item) => (
            <Card key={item.id} className="overflow-hidden border-0 shadow-lg card-hover-effect">
              <button
              ...
              >
                ...
                <CardContent className={`p-4 ${item.bgColor}`}>
                  <div className="mb-3 flex justify-between items-start">
                    <div className={`p-2 rounded-lg ${item.bgColor} ${item.textColor}`}>
                  ...
                  <h3 className={`font-semibold ${item.textColor}`}>{item.title}</h3>
          ...
          <CardContent className="p-4 bg-purple-50">
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-purple-700">{t("stories")}</h3>
              </div>
              <Badge className="bg-yellow-400/90 text-yellow-950">
