/**
 * Cookie utilities for client and server-side cookie management
 */
...
/**
 * Server-side cookie utilities using Next.js headers
 */
export class ServerCookies {
  ...
  static async set(name: string, value: string, options: CookieOptions = {}): Promise<void> {
    const { cookies } = await import("next/headers");
    const cookieStore = await cookies();
...
/**
 * Client-side cookie utilities for browser environment
 */
...
/**
 * Universal cookie utilities that work on both client and server
 */
export class UniversalCookies {
  ...
  static getCurrentChildId(): string | null {
    if (this.isServer()) {
      // On server, we can't access cookies synchronously in client components
      // This should be handled in server components or API routes
      return null;
    } else {
